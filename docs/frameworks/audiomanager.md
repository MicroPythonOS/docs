...
- `AudioManager.STREAM_ALARM` - Highest priority (value: 2)
- `AudioManager.STREAM_NOTIFICATION` - Medium priority (value: 1)
- `AudioManager.STREAM_MUSIC` - Lowest priority (value: 0)
...
3. **Hardware not available (desktop)**
    ...
    # AudioManager will return success but produce no sound
    ```
...
2. **Currently playing audio**
     ...
     AudioManager.record_wav("recording.wav")
     ```
